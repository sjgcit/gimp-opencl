...
/*
 * Kernel Sobel 3x3 edge filter
 *
 * $Id: sobel_w.cl,v 1.1 2014/11/05 10:05:01 sjg Exp $
 *
 * Has both an RGBA and SC kernel function
 *
 */

// menuentry Edge ( Sobel )
...
#define calc_g() \
    gx  = cols[2][0] + ( 2.0f*cols[2][1] ) + cols[2][2] ; \
    gx -= ( cols[0][0] + ( 2.0f*cols[0][1] ) + cols[0][2] ) ; \
    \
    gy  = cols[0][0] - cols[0][2] ; \
    gy += 2.0f*( cols[1][0] - cols[1][2] ) ; \
    gy += ( cols[2][0] - cols[2][2] ) ; \
    \
    g = sqrt( ( gx*gx ) + ( gy*gy ) ) ; \
    \
    g /= 6.0f ;
